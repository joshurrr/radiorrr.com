        ...
        adjusted["score"] = (
            ...
            if item.get("anti_genre_excluded") or item.get("speech_excluded")
                ...
                max(
                    0.0,
                    score
                    - float(item.get("anti_genre_penalty") or 0.0)
                    - float(item.get("speech_penalty") or 0.0)
                    + bpm_details["bpm_adjustment"],
                ),
                2,
            )
        )
...
async def switch_live_relay(dj: str = None):
    ...
    async with relay_lock:
        ...
        try:
            ...
            stream_info = await get_tiktok_stream(username)
            await validate_and_promote_relay(stream_info, username)
            ...
            print(
                f"[Relay] Manual global switch failed for @{username}: {error}"
            )
            raise HTTPException(
                status_code=503,
                detail=str(error),
...
def _admin_tiktok_username(value):
...
async def admin_add_dj(request: Request):
    """Add a TikTok DJ, or re-enable and refresh an existing DJ."""
    try:
        data = await request.json()
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid JSON request")

    username = _admin_tiktok_username(data.get("username"))

    try:
        profile = await import_tiktok_dj(username)
    except Exception as error:
        print(f"[Admin DJ] ADD ERROR @{username}: {error}")
...
async def admin_disable_dj(request: Request):
    """Disable a DJ while preserving the favourite/profile database row."""
    try:
        data = await request.json()
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid JSON request")
    ...
    try:
        ...
        if not row:
            raise HTTPException(status_code=404, detail=f"@{username} is not in the DJ database")
        ...
        # live_djs is only a live-state cache. Removing it here makes the
        # disable action take effect immediately while preserving the DJ's
        # stored favourite/profile and AI history.
        conn.execute(
            """
            DELETE FROM live_djs
            WHERE lower(username) = lower(?)
    ...
    profile_picture_refresh_attempted.discard(str(row["username"]))
...
async def admin_delete_dj(request: Request):
    """Permanently delete the favourite DJ row after typed DELETE confirmation."""
    try:
        data = await request.json()
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid JSON request")
    ...
    try:
        ...
        # Match the existing Radio RRR delete semantics: remove the DJ's
        # favourite/profile record. Live state is also cleared immediately.
        conn.execute(
            """
...
@media(max-width:760px){.top{align-items:flex-start;flex-direction:column}.form,.manage-grid,.scanner-controls{grid-template-columns:1fr}.panel-head{align-items:flex-start;flex-direction:column}.scanner-result-head{align-items:flex-start;flex-direction:column}}
</style>
</head>
<body>
<div class="wrap">
  ...
  <section class="panel" aria-labelledby="djManageTitle">
    ...
    <p class="copy">Manage a TikTok DJ using the same operational meanings as the NAS commands: add/re-enable, disable while keeping data, or permanently delete the favourite/profile record.</p>
    <div class="manage-grid">
      <div class="field"><label for="manageUsername">TikTok username</label><input id="manageUsername" placeholder="@username" autocomplete="off"></div>
      <button class="btn primary" type="button" onclick="addDj()">Add / Re-enable</button>
    ...
    <div class="note"><strong>Disable</strong> keeps the DJ's database/profile data. <strong>Delete</strong> permanently removes the favourite/profile record and requires typing DELETE to confirm.</div>
  </section>
...
function usernameFrom(id){const u=normalizeTikTokHandle($(id).value);if(!u)throw new Error('Enter a TikTok username.');return u}
function syncManageUsername(username){$('manageUsername').value='@'+username}
...
async function addDj(){let username;try{username=usernameFrom('manageUsername')}catch(e){return alert(e.message)}if(!confirm(`Add or re-enable @${username} in Radio RRR?`))return;try{const result=await api('/api/admin/djs/add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username})});const actual=result.username||username;$('manageUsername').value='@'+actual;toast(`Added / enabled @${actual}`)}catch(e){alert(e.message)}}
async function disableDj(){let username;try{username=usernameFrom('manageUsername')}catch(e){return alert(e.message)}if(!confirm(`Disable @${username}? Their database/profile data will be kept.`))return;try{const result=await api('/api/admin/djs/disable',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username})});toast(`Disabled @${result.username}`)}catch(e){alert(e.message)}}
let deleteUsername=null;
function confirmDeleteDj(){try{deleteUsername=usernameFrom('manageUsername')}catch(e){return alert(e.message)}$('deleteCopy').textContent=`This permanently removes @${deleteUsername}'s favourite/profile database record.`;$('deleteConfirmation').value='';$('overlay').classList.add('open');setTimeout(()=>$('deleteConfirmation').focus(),0)}
function closeDelete(){$('overlay').classList.remove('open');deleteUsername=null}
async function deleteDj(){if(!deleteUsername)return;const confirmation=$('deleteConfirmation').value.trim();if(confirmation!=='DELETE')return alert('Type DELETE exactly to confirm.');try{const result=await api('/api/admin/djs/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:deleteUsername,confirmation})});closeDelete();$('manageUsername').value='';toast(`Deleted @${result.username}`)}catch(e){alert(e.message)}}
